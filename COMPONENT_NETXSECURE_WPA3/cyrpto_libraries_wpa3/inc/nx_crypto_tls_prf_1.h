...
/* IFX: WPA3 Changes - Increase label size from 13 to 21 bytes */
typedef struct NX_CRYPTO_TLS_PRF_1_STRUCT
{
    NX_CRYPTO_PHASH nx_secure_tls_prf_phash_info;
    UCHAR nx_secure_tls_prf_label_seed_buffer[88]; /* phash_seed = label(21 bytes) || prf_seed(64 bytes) */
    UCHAR nx_secure_tls_prf_temp_A_buffer[108]; /* The temp_A buffer needs to be large enough to holdthe lable(21 bytes) || prf_seed(64 bytes) || hash_size(20 bytes for SHA1/MD5) */
