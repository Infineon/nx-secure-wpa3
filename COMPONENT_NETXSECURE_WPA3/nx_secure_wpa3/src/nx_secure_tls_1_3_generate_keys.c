...
UCHAR (*transcript_hashes)[NX_SECURE_TLS_MAX_HASH_SIZE] = tls_session->nx_secure_tls_key_material.nx_secure_tls_transcript_hashes;
    ...
    /* IFX: WPA3 Changes: Make sure we only derive the secrets once */
    if(secrets->tls_master_secret_len != 0 && secrets->tls_exporter_master_secret_len == 0)
    {
        ...
        /* IFX: WPA3 Changes: Set the correct length variable */
        secrets->tls_exporter_master_secret_len = hash_length;
    }
    else if (secrets->tls_master_secret_len != 0)
    {
        /* IFX: WPA3 Changes: Derive the resumption master secret at the end */
