...
UINT              metadata_size = 0;
    ...
    if (method_ptr  -> nx_crypto_operation != NX_NULL)
    {
        status = method_ptr  -> nx_crypto_operation(NX_CRYPTO_HASH_CALCULATE,
                                           ...
                                           hash_size,
                                           metadata,     
										   metadata_size,   /* IFX: WPA3 changes to add SHA384 */
                                           NX_NULL,
                                           NX_NULL);
    }
    ...
    if (need_copy)
    {
    	...
    	if (tls_session->nx_secure_tls_session_ciphersuite->nx_secure_tls_hash_size == 48)
    	{
    		NX_SECURE_HASH_CLONE_CLEANUP(metadata, tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata_size);
    	}
    	else
    	{
