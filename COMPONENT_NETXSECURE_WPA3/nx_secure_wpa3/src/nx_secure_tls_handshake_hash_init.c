...
static UINT _nx_secure_tls_handshake_hash_384_init (NX_SECURE_TLS_SESSION *tls_session);
...
UINT              metadata_size = 0;
    ...
    {
    	 /* IFX: WPA3 Changes : Add SHA384 hash initilialization */
    	_nx_secure_tls_handshake_hash_384_init (tls_session);
        ...
        else
        {
             ...
             if (tls_session->nx_secure_tls_session_ciphersuite->nx_secure_tls_hash_size == 48)
        	 {
        		 ...
        		 method_ptr = tls_session -> nx_secure_tls_crypto_table -> nx_secure_tls_handshake_hash_sha384_method;
				 metadata = tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata;
        	 }
        	 else
        	 {
        		 /* The handshake transcript hash in TLS 1.3 uses the hash routine associated with the chosen ciphersuite. */
        ...
        /* IFX: WPA3 changes to add SHA384 */
		metadata_size = (UINT)method_ptr->nx_crypto_metadata_area_size;
        if (method_ptr -> nx_crypto_init)
        {
            status = method_ptr -> nx_crypto_init((NX_CRYPTO_METHOD*)method_ptr,
                                         ...
                                         0,
                                         &handler,
										 metadata,  /* IFX: WPA3 changes to add SHA384 */
                                         metadata_size);
...
static UINT _nx_secure_tls_handshake_hash_384_init (NX_SECURE_TLS_SESSION *tls_session)
{
	...
	method_ptr = tls_session -> nx_secure_tls_crypto_table -> nx_secure_tls_handshake_hash_sha384_method;
    metadata = tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata;
    metadata_size = method_ptr->nx_crypto_metadata_area_size;
    ...
    if (method_ptr -> nx_crypto_init)
    {
       	...
       	tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_handler = handler;
    }
