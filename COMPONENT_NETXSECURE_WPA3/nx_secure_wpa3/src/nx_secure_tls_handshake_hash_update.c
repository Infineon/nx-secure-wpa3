...
static UINT _nx_secure_tls13_handshake_hash_update(NX_SECURE_TLS_SESSION *tls_session, UCHAR *data, 
                                                                      UINT length, const NX_CRYPTO_METHOD *method_ptr, 
                                                                      UINT metadata_size, VOID *metadata );
...
VOID                   *handler = NX_NULL;
    ...
    if(tls_session->nx_secure_tls_1_3)
    {
        ...
        if (tls_session -> nx_secure_tls_session_ciphersuite == NX_NULL)
        {
            ...
            /* IFX: WPA3 changes to add SHA384 */
		    /* Need to update the hash for both SHA256 and SHA384 since the remote peer could chose either one */
			...
			status = _nx_secure_tls13_handshake_hash_update(tls_session, data, length, method_ptr, metadata_size, metadata );
...
#endif /* NX_SECURE_ENABLE_DTLS */
    {
        ...
        if (tls_session -> nx_secure_tls_session_ciphersuite == NX_NULL)
        {
			...
			/* Set the hash method to the default of SHA-384 if no ciphersuite is available. */
            method_ptr = tls_session -> nx_secure_tls_crypto_table->nx_secure_tls_handshake_hash_sha384_method;
			metadata = tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata;
			metadata_size =  tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata_size;
...
#endif
#if (NX_SECURE_TLS_TLS_1_3_ENABLED)
   if (( tls_session == NX_NULL ) || (data == NX_NULL) || (length == 0) || (method_ptr == NX_NULL) 
		  || (metadata_size == 0) || (metadata == NX_NULL))
   {
       return (status);
   }
