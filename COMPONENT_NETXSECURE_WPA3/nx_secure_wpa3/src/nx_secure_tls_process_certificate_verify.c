...
/* IFX: WPA3 Changes */
static UCHAR handshake_hash[64 + 34 + NX_SECURE_TLS_MAX_HASH_SIZE]; /* We concatenate MD5 and SHA-1 hashes into this buffer, OR SHA-256 OR SHA-384. */
...
/* IFX: WPA3 Changes */
/* values picked up from https://www.rfc-editor.org/rfc/rfc8017#section-9.2 for SHA384 */
static const UCHAR _NX_SECURE_OID_SHA384[] = {0x30, 0x41, 0x30, 0x0d, 0x06, 0x09, 0x60, 0x86, 0x48, 0x01, 0x65, 0x03, 0x04, 0x02, 0x02, 0x05, 0x00, 0x04, 0x30};
#endif
#endif
...
NX_SECURE_EC_PUBLIC_KEY              *ec_pubkey;
...
#endif
    {
        ...
        if (client_certificate -> nx_secure_x509_public_algorithm == NX_SECURE_TLS_X509_TYPE_EC)
        {
            ...
            if (tls_session->nx_secure_tls_session_ciphersuite->nx_secure_tls_prf->nx_crypto_algorithm == NX_CRYPTO_PRF_HMAC_SHA2_384)
            {
                signature_algorithm = NX_SECURE_TLS_X509_TYPE_ECDSA_SHA_384;
            }
            else
    ...
    if(tls_session -> nx_secure_tls_1_3)
    {
         ...
         */
         ...
         {
             NX_SECURE_MEMCPY(&handshake_hash[64 + 34], transcript_hash, NX_SECURE_TLS_MAX_HASH_SIZE); /* Use case of memcpy is verified. */
             handshake_hash_length = sizeof(handshake_hash);
         }
         else
         {
    ...
    {
        /* IFX: WPA3 Changes */
        /* Check for crypto algorithm and set hash method*/
        if (tls_session->nx_secure_tls_session_ciphersuite->nx_secure_tls_prf->nx_crypto_algorithm == NX_CRYPTO_PRF_HMAC_SHA2_256)
        {
        ...
        {
            /* Calculate our final signature length for later offset calculations. */
            signature_length = 19 + 48; /* DER encoding (19) + SHA-384 hash size (48) */

            /* Generate a hash of all sent and received handshake messages to this point (not a Finished hash!). */
            /* Copy over the handshake hash state into a local structure to do the intermediate calculation. */
            NX_SECURE_HASH_METADATA_CLONE(tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_scratch, /*  lgtm[cpp/banned-api-usage-required-any] */
                                          tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata,
                                          tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata_size); /* Use case of memcpy is verified. */

            hash_method   = tls_session -> nx_secure_tls_crypto_table -> nx_secure_tls_handshake_hash_sha384_method;
            handler       = tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_handler;
            metadata_size = tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata_size;
        }
        if (hash_method -> nx_crypto_operation != NX_NULL)
        {
            status = hash_method -> nx_crypto_operation(NX_CRYPTO_HASH_CALCULATE,
...
#endif /* NX_SECURE_ENABLE_DTLS */
    {
	   ...
	   else
	   {
			...
			/* Signature size is the size of SHA-1 (20) */
			signature_length = 20;
    ...
    if (client_certificate -> nx_secure_x509_public_algorithm == NX_SECURE_TLS_X509_TYPE_RSA)
    {
...
#endif /* NX_SECURE_ENABLE_DTLS */
        {
            ...
            if ((packet_buffer[0] != NX_SECURE_TLS_HASH_ALGORITHM_SHA256 && 
				packet_buffer[0] != NX_SECURE_TLS_HASH_ALGORITHM_SHA384) ||
...
#endif /* NX_SECURE_ENABLE_DTLS */
        {
            ...
            if (tls_session->nx_secure_tls_session_ciphersuite->nx_secure_tls_prf->nx_crypto_algorithm == NX_CRYPTO_PRF_HMAC_SHA2_384)
		    {
				 ...
				 compare_value = NX_SECURE_MEMCMP(&working_ptr[0], _NX_SECURE_OID_SHA384, 19);
			
				 /* Check the handshake hash. */
				 compare_value += NX_SECURE_MEMCMP(&working_ptr[19], handshake_hash, 48);
            }
			else
			{
                /* Check the DER encoding. */
...
#endif /* NX_SECURE_ENABLE_DTLS */
        {
            ...
            working_ptr = &_nx_secure_decrypted_signature[data_size - signature_length];
             ...
             if (client_certificate -> nx_secure_x509_public_algorithm == NX_SECURE_TLS_X509_TYPE_EC)
             {
                ...
                compare_value = NX_SECURE_MEMCMP(working_ptr, handshake_hash, 20);
             }
			 else
			 {
			    /* Now put the data into the padded buffer - must be at the end. */
    ...
    else if (client_certificate -> nx_secure_x509_public_algorithm == NX_SECURE_TLS_X509_TYPE_EC)
    {
...
#endif /* NX_SECURE_ENABLE_DTLS */
        {
...
#endif
            {
                ...
                {
                    if(packet_buffer[0] != NX_SECURE_TLS_HASH_ALGORITHM_SHA384 ||
                       packet_buffer[1] != NX_SECURE_TLS_SIGNATURE_ALGORITHM_ECDSA)
                    {
#ifdef NX_SECURE_KEY_CLEAR
                        NX_SECURE_MEMSET(handshake_hash, 0, sizeof(handshake_hash));
                    ...
                    /* Hash size is SHA-384 hash size (48). */
                    data_size = 48;
                }
            }
...
#endif /* NX_SECURE_ENABLE_DTLS */
        {
            ...
            if (client_certificate -> nx_secure_x509_public_algorithm == NX_SECURE_TLS_X509_TYPE_EC)
            {
                 /* Hash size is the size of SHA-1 (20) */
                 data_size = 20;
            }
			else
			{
