...
/* IFX: WPA3 Changes */
static VOID _nx_secure_tls_packet_trim_trailing_zeros(NX_PACKET *packet_ptr);
        ...
        *bytes_processed += (ULONG)header_length + message_length;
            ...
            )
        {
            ...
            if(tls_session->nx_secure_tls_1_3)
            {
                ...
                {
                    /* IFX: WPA3 Changes
                     * We need to trim any trailing zero octets in the payload.
                     * RFC 8446, Section 5.4
                     */
                    _nx_secure_tls_packet_trim_trailing_zeros(decrypted_packet);
                    message_length = decrypted_packet -> nx_packet_length;
...
NX_PACKET *work_ptr;
    ...
    while (packet_ptr->nx_packet_length > 0)
    {
        /*
         * nx_packet_last is always set for the decrypt packet.
         */

        current_ptr = packet_ptr->nx_packet_last;

        /*
         * Use nx_packet_append_ptr and nx_packet_prepend_ptr to check the
         * valid data in this packet since nx_packet_length is the length
         * of valid data in the entire packet chain.
         */

        while (current_ptr->nx_packet_append_ptr > current_ptr->nx_packet_prepend_ptr)
        {
            if (current_ptr->nx_packet_append_ptr[-1] == 0)
            {
                current_ptr->nx_packet_append_ptr--;
        ...
        if (current_ptr->nx_packet_append_ptr != current_ptr->nx_packet_prepend_ptr)
        {
            break;
        }
        ...
        if (packet_ptr == current_ptr)
        {
            break;
        }

        for (work_ptr = packet_ptr; work_ptr->nx_packet_next != current_ptr; work_ptr = work_ptr->nx_packet_next)
        {
            /* Empty */
        }

        /*
         * Adjust the list and release the empty packet.
         */

        work_ptr->nx_packet_next = NULL;
