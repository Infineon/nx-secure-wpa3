...
NX_CRYPTO_EXTENDED_OUTPUT  extended_output;
    ...
    if (local_certificate -> nx_secure_x509_public_algorithm == NX_SECURE_TLS_X509_TYPE_RSA)
    {
...
#endif /* NX_SECURE_ENABLE_DTLS */
        {
		    ...
		    if (tls_session->nx_secure_tls_session_ciphersuite->nx_secure_tls_prf->nx_crypto_algorithm == NX_CRYPTO_PRF_HMAC_SHA2_384)
		    {
		       current_buffer[length]     = NX_SECURE_TLS_HASH_ALGORITHM_SHA384; 
		    }
			else
			{
			   current_buffer[length]     = NX_SECURE_TLS_HASH_ALGORITHM_SHA256;  
			}
			...
			if (tls_session->nx_secure_tls_session_ciphersuite->nx_secure_tls_prf->nx_crypto_algorithm == NX_CRYPTO_PRF_HMAC_SHA2_384)
            {
                ...
                NX_SECURE_MEMCPY(&working_ptr[0], _NX_SECURE_OID_SHA384, 19); /* Use case of memcpy is verified.  lgtm[cpp/banned-api-usage-required-any] */
			}
			else
			{
                /* Copy in the DER encoding. */
            ...
            if (tls_session->nx_secure_tls_session_ciphersuite->nx_secure_tls_prf->nx_crypto_algorithm == NX_CRYPTO_PRF_HMAC_SHA2_384)
            {
                ...
                NX_SECURE_MEMCPY(&working_ptr[19], handshake_hash, 48); /* Use case of memcpy is verified.  lgtm[cpp/banned-api-usage-required-any] */
            }
			else
			{
			    /* Now put the data into the padded buffer - must be at the end. */
...
#endif /* NX_SECURE_ENABLE_DTLS */
        {
            ...
            {
                /* Now put the data into the padded buffer - must be at the end. */
                NX_SECURE_MEMCPY(working_ptr, handshake_hash, 20); /* Use case of memcpy is verified. */
            }
            else
            {
            	/* Now put the data into the padded buffer - must be at the end. */
