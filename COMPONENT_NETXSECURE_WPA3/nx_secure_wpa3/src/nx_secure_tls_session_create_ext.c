...
/*IFX : WPA3 changes to add SHA384 support */
const NX_CRYPTO_METHOD *crypto_method_sha384;
ULONG metadata_size_sha384 = 0;
//ULONG nx_crypto_metadata_area_size  = 0;
#endif
	...
	if (crypto_method_sha256 != NX_NULL)
    {
		...
		/* IFX: WPA3 Changes to add SHA256 metadata_size */
	    max_handshake_hash_metadata_size += metadata_size_sha256;		
    }	
	
    /* IFX: WPA3 changes to add SHA384 method and size */
	crypto_method_sha384 = crypto_table -> nx_secure_tls_handshake_hash_sha384_method;
    if (crypto_method_sha384 != NX_NULL)
    {
        metadata_size_sha384 = crypto_method_sha384 -> nx_crypto_metadata_area_size;

        if (metadata_size_sha384 & 0x3)
        {
            metadata_size_sha384 += 4 - (metadata_size_sha384 & 0x3);
        }
		/* IFX: WPA3 Changes to add SHA384 metadata_size */
	    max_handshake_hash_metadata_size += metadata_size_sha384;		
    }
#endif
#if (NX_SECURE_TLS_TLS_1_3_ENABLED)
	...
	/* IFX: WPA3 changes to add SHA384 PRF method and size */
	if ((crypto_table -> nx_secure_tls_prf_sha384_method != NX_NULL) &&
        (max_tls_prf_metadata_size < crypto_table -> nx_secure_tls_prf_sha384_method -> nx_crypto_metadata_area_size))
    {
        max_tls_prf_metadata_size = crypto_table -> nx_secure_tls_prf_sha384_method -> nx_crypto_metadata_area_size;
    }
#endif
    ...
    /* IFX: WPA3 changes to add SHA384 metadata and size */
    tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata = &metadata_area[offset];
    tls_session -> nx_secure_tls_handshake_hash.nx_secure_tls_handshake_hash_sha384_metadata_size = metadata_size_sha384;
    offset += metadata_size_sha384;
#endif
