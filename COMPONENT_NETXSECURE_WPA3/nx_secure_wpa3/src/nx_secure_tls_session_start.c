...
NX_PACKET *send_packet;
    ...
    if (tls_session == NULL || cy_tcp_socket_send == NULL || cy_tcp_socket_receive == NULL || packet_pool == NULL ||
        (cy_tcp_ip_version != NX_IP_VERSION_V4 && cy_tcp_ip_version != NX_IP_VERSION_V6))
    {
    ...
    tls_session->cy_tcp_ip_version     = cy_tcp_ip_version;
    tls_session->cy_tcp_socket_send    = cy_tcp_socket_send;
    tls_session->cy_tcp_socket_receive = cy_tcp_socket_receive;
    tls_session->cy_tcp_opaque         = cy_tcp_opaque;
    ...
    /* Assign the TCP socket to the TLS session. */
    tls_session -> nx_secure_tls_tcp_socket = NULL;
    ...
    /* TLS Server of TLS Client session? */
    if (session_type_server)
    {
        /* This session is now being treated as a server - indicate that fact to the TLS stack. */
        tls_session -> nx_secure_tls_socket_type = NX_SECURE_TLS_SESSION_TYPE_SERVER;
    }
    ...
    {
        /* Our TLS session is a TLS Client. */
        tls_session -> nx_secure_tls_socket_type = NX_SECURE_TLS_SESSION_TYPE_CLIENT;
    }
