...
UINT status;
    ...
    /* IFX: WPA3 Changes */
    if (tls_session->cy_tcp_socket_send == NULL && tls_session->cy_tcp_socket_receive == NULL)
    {
    ...
    else
    {
        ...
        if (tls_session->cy_tcp_ip_version != NX_IP_VERSION_V4 && tls_session->cy_tcp_ip_version != NX_IP_VERSION_V6)
        {
            return(NX_SECURE_TLS_SESSION_UNINITIALIZED);
        }
    }
